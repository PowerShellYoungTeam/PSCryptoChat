...
<#
.SYNOPSIS
    Interactive encrypted chat between two terminals

.DESCRIPTION
    Terminal 1 (Host):   .\Chat.ps1 -Listen -Port 9000
    Terminal 2 (Client): .\Chat.ps1 -Connect -Peer localhost -Port 9000
...
if (-not $Listen -and -not $Connect) {
    ...
    Write-Host "  Host (Terminal 1):   .\Chat.ps1 -Listen -Port 9000" -ForegroundColor Gray
    Write-Host "  Client (Terminal 2): .\Chat.ps1 -Connect -Peer localhost -Port 9000" -ForegroundColor Gray
    Write-Host ""
    exit
}
...
# Create UDP client directly (not using UdpTransport to avoid Connect() issues)
